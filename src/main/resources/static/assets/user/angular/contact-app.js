...
app.controller("contact-ctrl", function($scope, $http) {
	...
	$scope.postComment = function() {
		...
		if (content == "" || name == "" || email == "") {
			...
			$("#notification").html(
				"<i class='fa fa-info-circle'></i> Vui lòng nhập đầy đủ nội dung!"
			);
			$("#notification").show();
		} else {
			...
			$("#notification").fadeIn(300).delay(3000).fadeOut(300); // Thêm fadeIn cho thông báo thành công
			...
			$http.post(`/rest/contact/form`, $scope.formComment).then((resp) => {
				...
				// Reset các trường input sau khi gửi thành công
				$("#input-review").val('');
				$("#input-name").val('');
				$("#input-email").val('');
			});
