...
$(document).ready(function() {
	...
	$("#quantity").keyup(function() {
		...
		if (quantity <= 0) {
			...
			$("#showErrorQuality").text("Vui lòng nhập số lượng sản phẩm!");
			checkQuantity = 10;
		} else {
...
app.controller("book-form-ctrl", function($scope, $http) {
	...
	$scope.load = function() {
		$http
			.get("/rest/book/form/" + $("#demo").val())
			.then((resp) => {
				$scope.form = resp.data;
				console.log("Book Data:", resp.data); // Kiểm tra dữ liệu từ API
				console.log("Publisher ID: ", $scope.form.publisher); 
			})
			.catch((error) => {
				console.log(error);
			});
	...
	$scope.update = function() {
		if (checkForm()) {
			...
			$http
				.put("/rest/book/form/" + $("#demo").val(), item)
				.then((resp) => {
					$scope.info.status = true;
					$scope.info.content = "Bạn đã cập nhật sản phẩm thành công!";
					$("#modalSuccess").modal("show");
					var path = "/admin/book/update/" + $scope.form.id;
					$("a").attr("href", path);
					console.log(resp);
				})
				.catch((error) => {
