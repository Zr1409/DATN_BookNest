...
app.controller("blog-form-ctrl", function($scope, $http) {
	$scope.form = {};
	$scope.info = {};
	$scope.itemBookCategory = [];
	$scope.initializeBookCategory = function() {
		$http.get("/rest/bookCategories").then((resp) => {

			$scope.itemBookCategory = resp.data;
		});
	};

	$scope.initializeBookCategory();
	...
	$scope.load = function() {
		$http
			...
			.then((resp) => {
				...
				console.log("Dữ liệu từ server:", resp.data); // Kiểm tra dữ liệu trả về từ API
				// Chuyển đổi uploadDay từ chuỗi sang Date
				$scope.form.uploadDay = resp.data.uploadDay ? new Date(resp.data.uploadDay) : null;
				// Chuyển đổi active thành Boolean để khớp với ng-model
				$scope.form.active = resp.data.active === true;  // Đảm bảo luôn là true/false
