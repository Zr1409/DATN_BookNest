...
$(document).ready(function () {
  $("#nameManufacturer").keyup(function () {
    ...
    if (name == "") {
      ...
      $("#showErrorNameManufacturer").text("Vui lòng nhập tên nhà xuất bản!");
  ...
  $("#describe").keyup(function () {
    ...
    if (describe == "") {
      ...
      $("#showErrorDescribe").text("Vui lòng nhập mô tả nhà xuất bản!");
      checkDescribe = 10;
    } else {
...
var app = angular.module("publisher-form-app", []);

app.controller("publisher-form-ctrl", function ($scope, $http) {
  $scope.form = {};
  $scope.info = {};
  $scope.save = function () {
    if (checkForm()) {
      var item = angular.copy($scope.form);
      $http.post(`/rest/publisher/form`, item).then((resp) => {
        $scope.info.status = true;
        $scope.info.content = "Bạn đã thêm mới thành công một nhà xuất bản!";
        $("#modalSuccess").modal("show");
        var path = "/admin/publisher/form";
        $("a").attr("href", path);
        console.log(resp);
      });
    }
