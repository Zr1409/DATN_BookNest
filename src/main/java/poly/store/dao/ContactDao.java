...
//	List<Comment> getListCommentByProductId(@Param("uid") Integer id);
	...
	@Query("SELECT COUNT(ct) FROM Contact ct WHERE ct.status = 0")
	int countPendingContacts();
	
	@Query("SELECT COUNT(ct) FROM Contact ct WHERE ct.status = 1")
	int countSuccessContacts();
